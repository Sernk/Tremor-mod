...
{
    public class EbonstoneChestTile : ModTile
    {
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            {
                // Если это первый клик, открываем сундук
                if (Main.editChest)
                {
                ...
                {
                    // В сетевой игре отправляем запрос на открытие сундука
                    NetMessage.SendData(MessageID.RequestChestOpen, -1, -1, null, left, top);
                    Main.stackSplit = 600;
                }
                else
                {
                    // В одиночной игре открываем сундук
                    ...
                    if (chestIndex != -1)
                    {
                        ...
                        {
                            // Если сундук уже открыт, закрываем его
                            player.chest = -1;
                            Recipe.FindRecipes();
        ...
        public override void MouseOver(int i, int j)
        {
            ...
            else
            {
                ...
                {
                    player.cursorItemIconID = ModContent.ItemType<Items.Placeable.EbonstoneChest>();
